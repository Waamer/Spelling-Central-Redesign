(() => {
  ...
  const DEFAULT_WORDS = ['jam', 'code', 'apple', 'swift', 'train', 'planet'];
  ...
  function pickWordsFromSelectedList() {
    try {
      const selected = JSON.parse(localStorage.getItem('spellingcentral_selected_list') || 'null');
      if (!selected || !Array.isArray(selected.words)) return [];
  ...
  function pickWordsFromBaseLibraries() {
    const baseLibraries = Array.isArray(window.wordLibraryData?.baseLibraries)
      ? window.wordLibraryData.baseLibraries
      : [];
    if (!baseLibraries.length) return [];

    // Keep fallback aligned with other games: use one library list, not all lists merged.
    const fallbackList = baseLibraries[Math.floor(Math.random() * baseLibraries.length)];
    return (Array.isArray(fallbackList?.words) ? fallbackList.words : [])
      .map(normalizeWord)
      .filter((word) => word.length >= 3 && word.length <= 12);
  }
  ...
  function buildGameWords() {
    const selectedWords = dedupeWords(pickWordsFromSelectedList());
    if (selectedWords.length) return selectedWords;

    const baseWords = dedupeWords(pickWordsFromBaseLibraries());
    if (baseWords.length) return baseWords;

    return DEFAULT_WORDS.map((word) => word.toLowerCase());
  }

  const WORDS = buildGameWords();
